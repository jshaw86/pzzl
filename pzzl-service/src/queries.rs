pub const INSERT_USERS_PUZZLE_STATEMENT: &str = "
            WITH input_data(name, media, email, puzzle_id, lat, lng) AS ( VALUES ($1, $2, $3, unique_short_id(), $4, $5)),
            ins_puzzle AS (
                ...
                ins_user AS (
                    ...
                    ins_user_puzzle AS (
                        INSERT INTO users_puzzles (user_id, puzzle_id, lat, lng) 
...
pub const UPDATE_USERS_PUZZLE_STATEMENT: &str = "
            WITH input_data(name, media, email, puzzle_id, lat, lng) AS ( VALUES ($1, $2, $3, $4, $5, $6)),
            ins_puzzle AS (
                INSERT INTO puzzles (id, name, media) 
                SELECT puzzle_id, name, media FROM input_data ON CONFLICT (id) DO UPDATE SET name=EXCLUDED.name, media=EXCLUDED.media RETURNING id AS puzzle_id, name, media),
                ins_user AS (
